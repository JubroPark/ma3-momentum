...
>
> **`fetch_history()`의 NaN이 실제로는 yfinance `period=` 버그였음 (2026-09-15 확인·수정)**: 정규 EOD 배치가 "NVDA 종가 값이 NaN" 오류로 중단(9/15, 9/12에도 같은 유형 재발). 처음엔 "yfinance 일시 장애"로 추정했으나 재현해보니 **Yahoo 서버엔 실제 종가가 멀쩡히 있는데, yfinance가 `period=`(또는 이전 행이 같이 딸려오는 넓은 `start`~`end`) 조회 시 최신 행만 NaN으로 잘못 파싱하는 라이브러리 버그**였음(NVDA/AAPL/MSFT/QQQ 전부 재현, `^IXIC`는 미재현 — 그래서 로그에 IXIC는 정상 조회됨). 요청 구간에 그 직전 유효 행이 하나라도 같이 포함되면 재현되고, "누락된 마지막 날짜만 딱"(`start=`마지막 유효일+1일) 요청하면 정상 값이 나옴 — 단순히 "최근 N일 재조회"로는 그 N일 구간에 직전 행이 딸려오면 똑같이 재현되므로 **반드시 마지막 유효일 다음 날부터로 좁혀야 함**. `fetch_history()`가 NaN 꼬리를 감지하면 그 지점부터만 재조회해 붙이도록 수정(장기 히스토리 자체는 그대로 유지 — MA200 등에 필요). `test_fetch_history.py`에 `yf.Ticker`를 스텁으로 교체한 회귀 테스트 있음. **주의**: `fetch_hedge_prices()`(TLT/IAU/GLD/TIP, `period="5d"`)도 같은 버그에 노출돼 있지만 이미 NaN이면 해당 티커만 `None`으로 넘어가게 처리돼 있어 배치를 죽이지 않음 — 아직 미수정.
>
...
- (2026-07-25 변경) 구간을 **그리드(2.5%/5%)에 고정된 정수**로 저장하면 프론트의 -25%/-50% 토글과 어긋나는 버그가 있어 폐기. 대신 `masam.rebalancing.nvda_lowest_close` / `qqq_lowest_close` / `rank2_lowest_close`(그리드 무관 원시 최저 EOD 종가)를 저장.
  ...
  - **1구간 하락 중 신고가만으로 저점·직전고점이 조기 리셋되던 버그 (2026-08-29 수정, 같은 날 2차 수정으로 보강)**: 1차 수정에서 `is_reset = reached_recovery or (new_peak and prev_zone == 0)`로 좁혀 `prev_zone==1`(2구간 하락 문턱 미달)일 때 신고가 단독으로 저점이 리셋되는 문제는 막았음. 하지만 더 근본적인 문제가 남아있었음: `base`(구간/`recovery_price` 계산 기준)를 여전히 **오늘 이미 갱신된 새 고점**(`new_high`)으로 계산하고 있었고, `entry["prev_high"]`도 `is_reset` 여부와 무관하게 매 배치 무조건 새 고점으로 갱신(506행)돼왔음 — 그 결과 활성 하락 사이클 도중(`prev_zone==1`) 가격이 살짝만 반등해 옛 직전고점을 넘어도 `prev_high`가 그 값으로 계속 밀려 올라가고, 회복 목표가(`recovery_price = base*(1-step*(prev_zone-2))`)도 매번 그 새 `base`를 따라 같이 올라가는 "움직이는 표적"이 됨 — 즉 2구간 회복 자체가 구조적으로 영영 불가능해짐(NVDA 실사례: 직전고점 225.3→저점 208.48(1구간)까지 하락 후 227.98로 반등했는데, `prev_high`가 227.98로 갱신되면 회복 목표가도 227.98*1.05=239.38로 밀려 올라가고, 다음에 또 살짝 신고가를 찍으면 목표가가 또 밀려 올라가는 식으로 반복). 매뉴얼상 재매수 트리거는 "막바지 2구간 상승"뿐, 신고가 경신이 아님. **수정**: `base`/`prev_zone`/`recovery_price`/`reached_recovery`를 전부 **어제까지의 직전고점**(`old_high`, 고정 기준점)으로 계산하도록 변경, `entry["prev_high"]`도 `is_reset`일 때만(즉 진짜 재매수가 일어났거나 `prev_zone==0`의 무해한 경우만) 갱신하고 그 외엔 `old_high`로 고정. `test_zone_reach.py`의 관련 헬퍼(`update`/`update_allin`/`update_since`) 전부 동일하게 맞춰 재검증. **소급 정정**: 배포 전 이미 오염된 NVDA `lowest_close`(227.98→208.48)와 `prev_high`/`nvda_prev_high`(227.98→225.3)를 `masam.json`에서 수동 정정.
  - **`base`가 allin으로 바뀌는 틈에 같은 버그가 재발 (2026-08-29 3차 수정)**: 2차 수정 직후 다음 EOD 배치에서 `prev_high`가 다시 227.98로 되돌아감. 원인은 `_base_of(close, allin, prev_high)`가 **오늘 종가**가 `allin`보다 낮아지면 `base`를 `prev_high`에서 `allin`으로 바꿔치기하는 기존 로직(이 함수 자체는 2026-08-29 이전부터 있던 설계, 프론트 `autoBaseFor`의 "직전고점/올인지점 탭 자동전환"과 같은 개념을 백엔드에서도 흉내낸 것) — NVDA 종가가 하루 217.55로 올인가(219.22) 밑으로 눌리자 `base`가 allin(219.22)으로 바뀌었고, 그 기준으로 재계산한 `prev_zone`이 우연히 0이 나옴(저점 208.48이 allin 기준 -5% 문턱(208.259) 바로 위라 걸리지 않음). `prev_zone==0`이면 "무해한 new_peak 리셋"이 허용되는데, since-윈도우 히스토리에서 매번 새로 계산되는 실제 과거 고점(227.98, 8/27 종가 — `old_high`는 얼려있어 225.3이지만 원시 가격 히스토리엔 여전히 남아있음)이 new_peak으로 잡히면서 저점·직전고점이 통째로 리셋됨. **수정**: `_base_of` 인라인화 + `used_prev_high = allin>0 and close>=allin` 불리언을 명시적으로 남겨서, "무해한 new_peak 리셋" 허용 조건에 `used_prev_high`(즉 base가 실제로 prev_high였을 때만)를 추가로 걸었음 — base가 allin으로 잡힌 날엔 신고가가 아무리 커도 무조건 저점·직전고점 유지. `test_zone_reach.py::update()`에 `allin`/`hist_max` 파라미터 추가해 이 시나리오(저점 위지만 종가만 잠깐 allin 밑으로 눌린 상태에서 과거 진짜 고점이 재계산되는 경우)를 정확히 재현하는 회귀 테스트 추가. **소급 정정**: 재발한 오염값(`lowest_close` 217.55→208.48, `prev_high`/`nvda_prev_high` 227.98→225.3)을 다시 수동 정정.
